import { pointsForDonkeyKongScore } from '@arcade/shared';

export interface DonkeyKongSubmission {
  score: number;
  elapsedMs: number;
}
...
// Same trade-off as space-invaders.ts/galaga.ts/asteroids.ts: real-time
// physics/platforming, no move list to replay, so this is a sanity-bound
// check on (score, elapsedMs) rather than an independent replay. See
// GitHub issue #15's scope notes.
const MIN_ELAPSED_MS_FOR_ANY_SCORE = 3000;
const MAX_SCORE = 999990;
const MAX_SCORE_PER_SECOND = 60;

export function validateDonkeyKongSubmission(submission: DonkeyKongSubmission): SubmissionVerdict {
  ...
  return { valid: true, points: pointsForDonkeyKongScore(score) };
}
