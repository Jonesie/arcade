import { pointsForAsteroidsScore } from '@arcade/shared';

export interface AsteroidsSubmission {
  score: number;
  elapsedMs: number;
}
...
// Same trade-off as space-invaders.ts/galaga.ts: real-time action, no move
// list to replay, so this is a sanity-bound check on (score, elapsedMs)
// rather than an independent replay. See GitHub issue #14's discussion.
const MIN_ELAPSED_MS_FOR_ANY_SCORE = 3000;
const MAX_SCORE = 999990;
const MAX_SCORE_PER_SECOND = 120;

export function validateAsteroidsSubmission(submission: AsteroidsSubmission): SubmissionVerdict {
  ...
  return { valid: true, points: pointsForAsteroidsScore(score) };
}
