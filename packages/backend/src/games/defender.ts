import { pointsForDefenderScore } from '@arcade/shared';

export interface DefenderSubmission {
  score: number;
  elapsedMs: number;
}
...
// action, no move list to replay, so this is a sanity-bound check on
// (score, elapsedMs) rather than an independent replay. See GitHub issue
// #4's discussion.
const MIN_ELAPSED_MS_FOR_ANY_SCORE = 3000;
const MAX_SCORE = 999990;
const MAX_SCORE_PER_SECOND = 150;

export function validateDefenderSubmission(submission: DefenderSubmission): SubmissionVerdict {
  ...
  return { valid: true, points: pointsForDefenderScore(score) };
}
