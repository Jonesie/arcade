import { pointsForGalagaScore } from '@arcade/shared';

export interface GalagaSubmission {
  score: number;
  elapsedMs: number;
}
...
// Same trade-off as space-invaders.ts: real-time action, no move list to
// replay, so this is a sanity-bound check on (score, elapsedMs) rather than
// an independent replay. See GitHub issue #2's discussion.
const MIN_ELAPSED_MS_FOR_ANY_SCORE = 3000;
const MAX_SCORE = 999990;
const MAX_SCORE_PER_SECOND = 90;

export function validateGalagaSubmission(submission: GalagaSubmission): SubmissionVerdict {
  ...
  return { valid: true, points: pointsForGalagaScore(score) };
}
