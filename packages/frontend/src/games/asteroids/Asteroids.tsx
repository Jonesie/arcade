...
export function Asteroids() {
  ...
  const inputRef = useRef<GameInput>({ left: false, right: false, thrust: false, fire: false });
  ...
  const submitMutation = useMutation({
    mutationFn: (payload: { score: number; elapsedMs: number }) => api.post<ScoreResponse>('/games/asteroids/scores', payload),
  });
  ...
  useEffect(() => {
    function handleKeyDown(e: KeyboardEvent) {
      ...
      if (e.key === 'ArrowUp' || e.key === 'w' || e.key === 'W') inputRef.current.thrust = true;
    ...
    function handleKeyUp(e: KeyboardEvent) {
      ...
      if (e.key === 'ArrowUp' || e.key === 'w' || e.key === 'W') inputRef.current.thrust = false;
  ...
  return (
    <div className={styles.game}>
      ...
      <div className={styles.screenWrap}>
        ...
        {status !== 'playing' && (
          <div className={styles.overlay}>
            {status === 'ready' && (
              <>
                <p>Arrow keys / A-D to rotate, W/Up to thrust, Space to fire.</p>
      ...
      <div className={styles.touchControls} aria-hidden="true">
        ...
        <button
          type="button"
          onPointerDown={() => press('thrust', true)}
          onPointerUp={() => press('thrust', false)}
          onPointerLeave={() => press('thrust', false)}
        >
          &#9650;
        </button>
        <button
