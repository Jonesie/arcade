...
import { createInitialState, HEIGHT, START_LIVES, update, WIDTH, type GameInput, type GameState } from './engine';
import { render } from './render';
import { ensureAudio, sfx, startMusic, stopMusic } from './sound';
import styles from './DonkeyKong.module.scss';
...
const EMPTY_INPUT: GameInput = { left: false, right: false, up: false, down: false, jump: false };
...
export function DonkeyKong() {
  ...
  const submitMutation = useMutation({
    mutationFn: (payload: { score: number; elapsedMs: number }) =>
      api.post<ScoreResponse>('/games/donkey-kong/scores', payload),
  });
      ...
      <div className={styles.screenWrap}>
        ...
        {status !== 'playing' && (
          <div className={styles.overlay}>
            {status === 'ready' && (
              <>
                <p>Arrows/WASD to move, Up/Down to climb ladders, Space to jump barrels.</p>
